/*------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Description: Contains the full encoding pipeline for BMP-based LSB steganography. Handles input validation, capacity checks, header copying, and staged
              embedding of the magic string, file extension, extension size, secret file size, and secret file contents. Uses 8-byte LSB packing for each
              encoded character and 32-byte LSB packing for integer values. Produces a complete stego BMP while preserving remaining image data.
------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
    ...
    // validate wheather user has provided  .bmp or not
    ...
    // validate wheather user has provided  .txt or not
...
Status encode_data_to_image(const char *data, int size, FILE *fptr_src, FILE *fptr_stego, EncodeInfo *encInfo)
{
    for (int i = 0; i < size; i++)
    {
        ...
        // call encode lsb to byte to encode the data to lsb
...
{
    // al the char encoding will call encode sata to image
...
Status encode_size(uint size, FILE *fptr_src, FILE *fptr_stego){
    char rgb[32];
    //read 32 bytes from sourse image
...
Status encode_secret_file_data(EncodeInfo *encInfo){
    ...
    for (int i = 0; i < encInfo -> size_secret_file; i++){
        ...
        //call encode byte to lsb to encode the secret data
            ...
            if (copy_bmp_header(encInfo->fptr_src_image, encInfo->fptr_stego_image) == e_success)
            {
                ...
                if (encode_magic_string(MAGIC_STRING, encInfo) == e_success)
                {
                    ...
                    if (encode_size(strlen(".txt"), encInfo->fptr_src_image, encInfo->fptr_stego_image) == e_success)
                        {
                            printf("Successfully encoded file extension size!\n");
                            if (encode_secret_file_extn(".txt", encInfo) == e_success){
                                ...
                                if(encode_secret_file_size(encInfo -> size_secret_file, encInfo) == e_success){
                                    ...
                                    // RESET secret file pointer after size calculation
